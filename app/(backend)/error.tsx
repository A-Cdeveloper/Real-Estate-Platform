...
export default function Error({
  ...
  useEffect(() => {
    ...
    console.error("Backend error caught by boundary:", error);
  }, [error]);

  return (
    <div className="min-h-[calc(100vh-200px)]  bg-background flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-card border border-border rounded-lg p-8 text-center shadow-lg">
        ...
        <div className="flex gap-4 justify-center">
          ...
          <Button variant="outline" asChild className="font-nunito-sans">
            <Link href="/proprietes-area">Back to properties</Link>
          </Button>
        </div>
      </div>
    </div>
